...
namespace Jde{
	...
	α UserTests::SetUpTestCase()->void{
		...
		if( DB::Scaler<uint>(Ƒ("select count(*) from um_providers where target='{}'", OpcServer))==0 )
			DB::Execute( Ƒ("insert into um_providers(provider_type_id, target) values({}, '{}')", (uint)UM::EProviderType::OpcServer, OpcServer), {} );
		OpcProviderId = DB::Scaler<uint>( Ƒ("select id from um_providers where target='{}'", OpcServer) ).value_or(-1);
	}
	...
	α CreateUser( str name, uint providerId=(uint)UM::EProviderType::Google )->UserPK{
		var create = Ƒ( "{{ mutation createUser(  'input': {{'loginName':'{0}','target':'{0}','provider':{1},'name':'{0} - name','description':'{0} - description'}} ){{id}} }}", name, providerId );
	...
	α PurgeUser( UserPK userId )->void{
		var purge = Ƒ( "{{mutation purgeUser(\"id\":{}) }}", userId );
		var purgeJson = DB::Query( purge, 0 );
		TRACE( "purgeJson={}", purgeJson.dump() );
	}

	α SelectUser( string target )->json{
		var selectArray = Ƒ( "query{{ user(target:[\"{}\"]){{id loginName provider{{id name}}}} }}", target );
	...
	TEST_F( UserTests, Crud ){
		...
		var read = Ƒ("query{{ user(filter:{{target:{{ eq:\"{}\"}}}}){{ id name attributes created updated deleted target description isGroup provider{{ id name }} }} }}", user );
		var readJson = DB::Query( read, 0 );
		TRACE( "{}", readJson.dump() );
		var readId = readJson["data"]["user"]["id"].get<int>();
		ASSERT_EQ( id, readId );

		var update = Ƒ( "{{ mutation updateUser( \"id\":{}, \"input\": {{\"name\":\"{}\"}} ) }}", readId, "newName" );
		var updateJson = DB::Query( update, 0 );
		TRACE( "{}", updateJson.dump() );

		var del = Ƒ( "{{mutation deleteUser(\"id\":{}) }}", readId );
	...
	TEST_F( UserTests, MultipleUsersSelect ){
		...
		var q = Ƒ( "query{{ users(id:[{},{}]){{id loginName provider{{id name}}}} }}", a, b );
		auto j = DB::Query( q, 0 );
		TRACE( "{}", j.dump() );
		PurgeUser( a );
		PurgeUser( b );
