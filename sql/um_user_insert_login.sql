...
#DELIMITER $$
create procedure um_user_insert_login( _login_name varchar(255), provider_id int unsigned,_provider_target varchar(255) )
begin
	...
	if( _provider_target is not null ) then
		select id into provider_id from um_providers where target = _provider_target;
		set provider_name = _provider_target;
	else
		set provider_id = provider_id;
		if( provider_id is not null ) then
			select name into provider_name from um_provider_types where id = provider_id;
		end if;
	end if;
	...
	CALL um_entity_insert(_login_name, 0, provider_target, null, false, provider_id);
