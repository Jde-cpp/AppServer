create or alter proc um_user_insert_login( @login_name varchar(255), @provider_id int, @provider_target varchar(255) ) as
begin
	declare @entity_id int; declare @provider_name varchar(255);

	if @provider_target is not null begin
		select @provider_id=id from um_providers where target = @provider_target;
		set @provider_name = @provider_target;
	end
	else begin
		if @provider_id is not null begin
			select @provider_name=name from um_provider_types where id = @provider_id;
		end
	end
